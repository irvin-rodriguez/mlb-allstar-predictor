...
for epoch in range(num_epochs):
    ...
    for inputs, labels in train_loader:
        ...
        def closure(backward=True):
            global forward_passes, backward_passes
            ...
            if loss.requires_grad and backward:
                loss.backward(retain_graph=True, create_graph=True)  # back propagation
                backward_passes += 1

            return loss
        
        tr_loss = optimizer.step(closure=closure)  # optimizer step
        running_tr_loss += tr_loss.item()

        # Computing Accuracy
        with torch.no_grad():
            preds = torch.sigmoid(model(inputs))  # convert logits into predictions
...
plt.legend()
plt.tight_layout()
plt.savefig("outputs/TRNCG_train_val_loss.png")
plt.close()
...
plt.tight_layout()
plt.savefig("outputs/TRNCG_confusion_matrix_raw.png")
plt.close()
...
plt.tight_layout()
plt.savefig("outputs/TRNCG_confusion_matrix_normalized.png")
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("outputs/TRNCG_roc_curve.png")
plt.close()
...
plt.tight_layout()
plt.savefig("outputs/TRNCG_precision_recall_curve.png")
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("outputs/TRNCG_epoch_loss_vs_propagations.png")
plt.close()
